...
func NewQuerier(k Keeper) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err error) {
		...
		default:
			return nil, sdkerror.Wrap(sdkerror.ErrUnknownRequest, "unknown staking query endpoint")
		}
	}
}
...
func queryPool(ctx sdk.Context, k Keeper) ([]byte, error) {
	...
	if bondedPool == nil || notBondedPool == nil {
		return nil, sdkerror.Wrap(sdkerror.ErrInternal, "pool accounts haven't been set")
	}

	pool := types.NewPool(
		k.bankKeeper.GetAllBalances(ctx, notBondedPool.GetAddress()).AmountOf(bondDenom),
		k.bankKeeper.GetAllBalances(ctx, bondedPool.GetAddress()).AmountOf(bondDenom),
	)
...
func queryValidatorAllShares(ctx sdk.Context, req abci.RequestQuery, k Keeper) ([]byte, error) {
	...
	if err := types.ModuleCdc.UnmarshalJSON(req.Data, &params); err != nil {
		return nil, sdkerror.Wrap(sdkerror.ErrInternal, fmt.Sprintf("failed to parse validator params. %s", err))
	}
...
func queryForAddress(ctx sdk.Context, req abci.RequestQuery, k Keeper) (res []byte, err error) {
	...
	if len(validatorAddr) != crypto.AddressSize*2 {
		return nil, sdkerror.ErrInvalidAddress
	}
...
func queryForAccAddress(ctx sdk.Context, req abci.RequestQuery) (res []byte, err error) {
	...
	if errBech32 != nil {
		return nil, sdkerror.Wrap(sdkerror.ErrInternal, "failed to get operator address" + errBech32.Error())
	}
...
func defaultQueryErrJSONMarshal(err error) error {
	return sdkerror.Wrap(sdkerror.ErrInternal, "failed to marshal result to JSON" + err.Error())
}

func defaultQueryErrParseParams(err error) error {
	return sdkerror.Wrap(sdkerror.ErrInternal, fmt.Sprintf("failed to parse params. %s", err))
}
