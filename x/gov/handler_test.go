...
func TestHandleMsgVote3(t *testing.T) {
	...
	require.Equal(t, proposalCoins, gk.BankKeeper().GetAllBalances(ctx,
	...
	res, err = govHandler(ctx, newVoteMsg)
	require.Nil(t, err)
	require.Equal(t, sdk.Coins(nil), gk.BankKeeper().GetAllBalances(ctx,
		ak.GetModuleAccount(ctx, types.ModuleName).GetAddress()))
}
...
func TestHandleMsgSubmitProposal(t *testing.T) {
	...
	proposalCoins = sdk.DecCoins{sdk.NewInt64DecCoin(sdk.DefaultBondDenom, 500)}
	content = types.NewTextProposal("Test", "description")
	newProposalMsg, err = NewMsgSubmitProposal(content, proposalCoins, sdk.AccAddress{})
	require.Nil(t, err)
	_, err = handler(ctx, newProposalMsg)
	require.NotNil(t, err)
