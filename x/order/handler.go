...
func handleNewOrder(ctx sdk.Context, k Keeper, sender sdk.AccAddress,
	...
	if err != nil {
		code = sdkerror.ErrUnknownRequest.Error()
	} else {
		if k.IsProductLocked(ctx, msg.Product) {
			code = sdkerror.ErrInternal.Error()
			...
			code = sdkerror.ErrInsufficientFunds.Error()
		}
	}
...
func handleMsgNewOrders(ctx sdk.Context, k Keeper, msg types.MsgNewOrders,
	...
	if handlerResult.None() {
		return nil, sdkerror.ErrInternal
	}
...
func ValidateMsgNewOrders(ctx sdk.Context, k keeper.Keeper, msg *types.MsgNewOrders) error {
	...
	for _, item := range msg.OrderItems {
		...
		if k.IsProductLocked(ctx, msg.Product) {
			return fmt.Errorf("the trading pair (%s) is locked, please retry later", msg.Product)
		}
...
func validateCancelOrder(ctx sdk.Context, keeper keeper.Keeper, msg types.MsgCancelOrder) error {
	...
	// Check order
	if order == nil {
		return fmt.Errorf("order(%s) does not exist or already closed", msg.OrderID)
	}
	if order.Status != types.OrderStatusOpen {
		return fmt.Errorf("cannot cancel order with status(%d)", order.Status)
	}
	if !order.Sender.Equals(msg.Sender) {
		return fmt.Errorf("not the owner of order(%v)", msg.OrderID)
	}
	if keeper.IsProductLocked(ctx, order.Product) {
		return fmt.Errorf("the trading pair (%s) is locked, please retry later", order.Product)
	}
	return nil
}
